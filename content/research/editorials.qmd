...
For the most up-to-date listing of my work, please see my [CV](/content/CV.pdf), [Google Scholar](https://scholar.google.com/citations?user=XUH1C6YAAAAJ&hl=en), or [My Bibliography](https://www.ncbi.nlm.nih.gov/myncbi/stephen.salerno.2/bibliography/public/).
...
bib  <- ReadBib("editorials.bib", check = FALSE)
keys <- names(bib)
...
rows <- tibble(
  ...
  mutate(
    ...
    url_fallback = normalize_link(url_fallback),
...
build_card <- function(
  ...
  ttl_node <- if (isTRUE(nzchar(doi_url %||% ""))) {
    tags$div(class = "pub-title", tags$a(title, href = preprint_url, target = "_blank", rel = "noopener"))
  } else {
  ...
  actions <- tags$div(
    ...
    if (isTRUE(primary)) tags$span(
      class = "lab-star",
      role  = "img",
      title = "First or co-first author",
      `aria-label` = "First or co-first author",
      "★",
      tags$span(class = "sr-only", " First or co-first author")
    ),
    # Action links
    ...
    if (isTRUE(has_preprint_key))
      tags$a("Free Text", class = "external", href = (preprint_url %||% "#"), target = "_blank", rel="noopener"),
...
This page was adapted from [https://github.com/jmgirard/affcomlab/blob/main/publications.qmd](https://github.com/jmgirard/affcomlab/blob/main/publications.qmd)
